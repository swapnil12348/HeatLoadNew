    ...
    >
      <td className="px-6 py-3 whitespace-nowrap">
        <div className="flex items-center">
          <div className={`w-1 h-8 rounded-l-md mr-3 ${ahu ? 'bg-blue-500' : 'bg-slate-300'}`} />
          <div>
      ...
      {/* BUG-04: floorArea is in m² — label corrected */}
      <td className="px-6 py-3 text-sm text-slate-600 font-mono">
        {roomData.floorArea ? roomData.floorArea.toLocaleString() : '—'}{' '}
...
export default function RDSPage() {
  ...
  return (
    // BUG-16 FIX: was h-[calc(100vh-64px)] — only subtracted header height.
    // Now h-full which fills the flex-1 main container in AppLayout exactly.
    // AppLayout's <main> is flex-1 overflow-auto, so this fills the remaining
    // viewport height after Header + TabNav regardless of their actual pixel height.
    <div className="flex h-full bg-slate-50 relative overflow-hidden">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${selectedRoomId ? 'mr-[600px]' : ''}`}>
        ...
        {/* Header */}
        <div className="bg-white px-8 py-5 border-b border-slate-200 flex justify-between items-center shadow-sm z-10 shrink-0">
          <div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-white border border-slate-300 text-slate-700 px-4 py-2 rounded-lg text-sm font-bold shadow-sm hover:bg-slate-50"
            >
              + System
            </button>
            <button
              onClick={() => dispatch(addNewRoom())}
              className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-md hover:bg-blue-700 hover:shadow-lg transition-all"
            >
              + Add Room
            </button>
          </div>
                ...
                <div key={ahuId} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-6 py-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <div className={`w-3 h-3 rounded-full ${ahu ? 'bg-blue-500' : 'bg-slate-300'}`} />

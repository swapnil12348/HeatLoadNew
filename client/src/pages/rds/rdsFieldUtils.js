 ...
 *  - flat keys                         → room[key]
 */
export const getFieldValue = (col, room, envelope, rdsRow = null) => {
  if (!room) return '';
  if (col.isEnv) {
    return envelope?.internalLoads?.[col.envType]?.[col.envField] ?? 0;
  }
  // Derived/readOnly fields live on the rdsRow (selectRdsData), not the raw room.
  // Raw room only has editable fields — computed outputs are never stored there.
  if (col.derived && rdsRow) {
    return rdsRow[col.key] ?? 0;
  }
  if (col.key.includes('.')) {
...
export const buildRoomUpdate = (col, rawValue) => ({
  ...
  value: (col.type === 'select' || col.inputType === 'text')
  ? rawValue

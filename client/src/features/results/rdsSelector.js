...
const selectHumidificationTarget = (state) => state.project.systemDesign.humidificationTarget ?? 45;
...
export const selectRdsData = createSelector(
  [selectRooms, selectEnvelopes, selectAhus, selectClimate, selectSystemDesign, selectElevation, selectLatitude, selectDailyRange, selectHumidificationTarget],
  (rooms, envelopes, ahus, climate, systemDesign, elevation, latitude, dailyRange, humidificationTarget) => {
    ...
    return rooms.map(room => {
      ...
      // BUG-14 FIX: safety factor and fan heat were previously compounded.
      //
      // OLD (wrong): grandTotal = (peakErsh + peakErlh) * fanHeatMult
      //   peakErsh and peakErlh already include safetyMult from calculateSeasonLoad.
      //   Multiplying by fanHeatMult on top of safety-inflated loads meant the
      //   fan heat allowance was itself inflated by the safety factor — double-dipping.
      //
      // CORRECT per ASHRAE:
      //   1. Room loads (ERSH, ERLH) carry the safety factor — accounts for
      //      calculation uncertainty in the room load estimate.
      //   2. Fan heat is a KNOWN physical addition to the system — it is the
      //      heat added by the supply fan motor to the airstream.
      //      It should NOT be inflated by the safety factor.
      //   3. grandTotal = ERSH + ERLH + fanHeatBTU
      //      where fanHeatBTU = (ERSH + ERLH) × fanHeatFraction
      //      This is equivalent to grandTotal = (ERSH + ERLH) × (1 + fanHeatFraction)
      //      but starting from the safety-included loads, not raw loads.
      //
      // The difference matters at high safety factors:
      //   10% safety, 5% fan: old = 1.10 × 1.05 = 1.155×  error: +0.5%
      //   20% safety, 8% fan: old = 1.20 × 1.08 = 1.296×  error: +1.6%
      //   30% safety, 10% fan: old = 1.30 × 1.10 = 1.430× error: +3.0%
      //
      const fanHeatFraction = (systemDesign.fanHeat || 5) / 100;
      const fanHeatBTU_room = (peakErsh + peakErlh) * fanHeatFraction;
      const grandTotal = (peakErsh + peakErlh) + fanHeatBTU_room;
      ...
      // BUG-14 FIX: use the directly computed fanHeatBTU_room, not back-calculated
      // from grandTotal. The back-calculation was only approximate and could
      // drift if grandTotal formula changed.
      const supplyFanHeatBlow = Math.round(fanHeatBTU_room);
      const supplyFanHeatDraw = (fanHeatBTU_room / ASHRAE.KW_TO_BTU).toFixed(2);
      ...
      //
      // When outdoor grains < indoor grains the infiltration latent load is
      // negative — the dry outdoor air REMOVES moisture from the room.
      // Without humidification the room will drop below its target RH.
      // This is the dominant HVAC load in:
      //   - Pharmaceutical facilities (target RH 30–60%)
      //   - Semiconductor fabs (target RH 40–50%)
      //   - Battery dry rooms (target RH <5% — extreme case)
      //   - Any cold climate in winter
      //
      // Calculation (ASHRAE sensible/latent analogy):
      //   grIn_target = calculateGrains(dbInF, humidificationTarget, elevation)
      //   grOut_winter = outdoor winter grains at site elevation
      //   ΔGr = grIn_target − grOut_winter  (positive = must add moisture)
      //
      //   Humidification CFM = supplyAir (the air volume that needs to be raised
      //     to the target humidity — use supply air as the controlled volume)
      //
      //   Humidification load (BTU/hr) = Cl × supplyAir × ΔGr
      //     where Cl = 0.68 × altCf (latent psychrometric factor)
      //
      //   Humidification load (lbs/hr water) = supplyAir × ΔGr / 7000
      //     (7000 grains per lb of water)
      //
      // If ΔGr ≤ 0 (outdoor air is already humid enough) → no humidification needed.
      // This can happen in warm/humid winter climates (monsoon shoulder season).
      //
      ...
      // Target indoor grains at humidification set-point
      const humidGrTarget = calculateGrains(dbInF, humidificationTarget, elevation);

      // Moisture deficit: how many grains/lb must we add to reach target RH
      const humidDeltaGr = Math.max(0, humidGrTarget - winterGrOut);

      // Humidification load in BTU/hr (latent heat of vaporization basis)
      const Cl_winter = ASHRAE.LATENT_FACTOR * altCf;
      const humidLoadBTU = supplyAir > 0
        ? Math.round(Cl_winter * supplyAir * humidDeltaGr)
        : 0;

      // Humidification capacity in lbs/hr of steam or water required
      // 1 lb water = 7000 grains; 1 lb steam ≈ 970 BTU latent heat (at 212°F)
      const humidLbsPerHr = supplyAir > 0
        ? ((supplyAir * humidDeltaGr) / 7000).toFixed(2)
        : '0.00';

      // kW of electric humidifier (steam electrode): Q_kW = lbs/hr × 0.634
      // 1 lb/hr steam ≈ 634 W (ASHRAE steam humidifier sizing standard)
      const humidKw = ((parseFloat(humidLbsPerHr) * 0.634)).toFixed(2);

      // Flag: does this room require humidification at all?

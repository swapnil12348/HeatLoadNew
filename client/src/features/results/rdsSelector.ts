...
// @ts-ignore - Ignore missing types until seasonalLoads.js is converted
import { calculateSeasonLoad } from './seasonalLoads';
// @ts-ignore - Ignore missing types until airQuantities.js is converted
...
// Stubbing RootState to provide strong typing for the selectors
  ...
  (
    ...
    return rooms.map((room) => {
      try {
        ...
        const oaFields: Record<string, number | string> = {};
        SEASONS_LIST.forEach((season) => {
        ...
        const coilLoadBTU =
          peakErshForCap + peakErlhForCap + oaPeak.oaTotal + returnFanHeatBTU;
        ...
        const adpSufficient =
          eshfType === 'no_solution'
            ? 'no_solution'
            : eshfType === 'sensible_only'
            ? 'not_applicable'
            : adpGap === null
            ? 'not_applicable'
            : adpGap <= 0
        ...
        // ════════════════════════════════════════════════════════════════════════
        const recircFraction =
          finalSupplyAir > 0 ? finalReturnAir / finalSupplyAir : 0;
        ...
        // ════════════════════════════════════════════════════════════════════════
        const pickupFields: Record<string, string> = {};
        const achFields: Record<string, string> = {};
        const termHeatFields: Record<string, string> = {};
        ...
        SEASONS_LIST.forEach((s) => {
          ...
          pickupFields[`pickupOn_${s}`] =
            finalSupplyAir > 0 ? (e_on / (Cs * finalSupplyAir)).toFixed(1) : '0.0';
          pickupFields[`pickupOff_${s}`] =
            finalSupplyAir > 0 ? (e_off / (Cs * finalSupplyAir)).toFixed(1) : '0.0';
          ...
          termHeatFields[`termHeatOn_${s}`] =
            e_on < 0 ? (Math.abs(e_on) / KW_TO_BTU_HR).toFixed(2) : '0.00';
          termHeatFields[`termHeatOff_${s}`] =
            e_off < 0 ? (Math.abs(e_off) / KW_TO_BTU_HR).toFixed(2) : '0.00';
        });

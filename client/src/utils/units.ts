/**
 * units.ts
 * Responsibility: Unit conversion helpers used across the entire codebase.
 *
 * All functions are pure — no side effects, no imports.
 * Organised by conversion family so callers can import only what they need.
 *
 * Duplication audit — these conversions were previously scattered across:
 *   rdsSelector.js      — M2_TO_FT2, M3_TO_FT3 inline constants
 *   heatingHumid.js     — BTU/kW inline
 *   airQuantities.js    — CFM/m³ inline
 *   AHUConfig.jsx       — M2_TO_FT2 inline constant
 *   ResultsPage.jsx     — M2_TO_FT2 inline constant
 *   RDSPage.jsx         — no conversion (was displaying wrong unit)
 *
 * After this file: all of the above import from here instead.
 *
 * ── NAMING CONVENTION ─────────────────────────────────────────────────────────
 *
 *   xToY(value)   — converts a single value from unit X to unit Y
 *   xFromY(value) — inverse of xToY (alias pattern)
 *
 *   Constants exported as SCREAMING_SNAKE_CASE for non-function contexts
 *   (e.g. column widths, display strings, table lookups).
 *
 * ── INPUT GUARD PATTERN ───────────────────────────────────────────────────────
 *
 *   Temperature (and other signed) conversions use an explicit NaN/null guard
 *   rather than `|| 0`.
 *
 *   The `|| 0` pattern silently substitutes 0 for invalid input. For
 *   temperature this produces a physically meaningful but wrong result:
 *     fToC(null) → (0 − 32) × 5/9 = −17.78°C  ← wrong, not NaN
 *
 *   Conversions that have a meaningful zero (airflow, area, power) keep
 *   `parseFloat(x) || 0` because 0 is a safe default for those units.
 *   Conversions where 0 is a meaningful design value (temperature) use
 *   the guard below and return null for invalid input.
 *
 *   const safeTemp = (v) => { const n = parseFloat(v); return isNaN(n) ? null : n; };
 *
 * ── CHANGELOG v2.0 ────────────────────────────────────────────────────────────
 *
 *   BUG-UNITS-01: Added g/kg ↔ gr/lb converters (gPerKgToGrPerLb etc.)
 *     ASHRAE Fundamentals tables express W in g/kg; app uses gr/lb internally.
 *     Ad-hoc factor 0.437 found in two places was WRONG — correct is ×7 exactly.
 *
 *   BUG-UNITS-02: Added Pa ↔ hPa converters (hpaToPa, paToHpa).
 *     Bridges psychro.js internals (hPa) with SI output layer.
 *
 *   BUG-UNITS-03: Added kg/kg → gr/lb converter (kgPerKgToGrPerLb).
 *     kg/kg and lb/lb are numerically identical. ×7000 exactly.
 *
 *   BUG-UNITS-04: Added hPa ↔ inHg converters (hpaToInHg, inHgToHpa).
 *     Bridges sitePressure() output with ASHRAE climate data format.
 *
 *   Added: lbPerHrToKgPerHr / kgPerHrToLbPerHr — for humidifier capacity sizing.
 *   Added: PSYCHRO_SANITY_CHECKS — reference values for regression testing.
 *
 *   PSYCHRO_SANITY_CHECKS v2.1 corrections (see bottom of file):
 *     reference.enthalpyBtuLb: 28.10 → 25.30  (28.10 is correct for 75°F, not 70°F)
 *     denver.sensibleFactor:    0.899 → 0.889  (correct for altCf at 5280 ft)
 *     denver.grains:           23.6  → 58.2   (computed with altitude-corrected P_site)
 */

// ══════════════════════════════════════════════════════════════════════════════
// INTERNAL GUARD HELPERS
// ══════════════════════════════════════════════════════════════════════════════

/** Safe parse for values where 0 is a valid meaningful result (area, flow, power). */
export const num = (v: number | string | null | undefined): number => {
  if (v === null || v === undefined) return 0;
  return parseFloat(String(v)) || 0;
};

/**
 * Safe parse for signed values where invalid input must NOT default to 0.
 * Returns null for NaN/undefined/null input so callers can detect bad data.
 * Used for temperature conversions.
 */
const numOrNull = (v: number | string | null | undefined): number | null => {
  if (v === null || v === undefined) return null;
  const n = parseFloat(String(v));
  return isNaN(n) ? null : n;
};

// ══════════════════════════════════════════════════════════════════════════════
// AREA
// ══════════════════════════════════════════════════════════════════════════════

/** 1 m² = 10.7639 ft² (exact per NIST) */
export const M2_TO_FT2 = 10.7639;

export const m2ToFt2 = (m2: number | string): number => num(m2) * M2_TO_FT2;
export const ft2ToM2 = (ft2: number | string): number => num(ft2) / M2_TO_FT2;

// ══════════════════════════════════════════════════════════════════════════════
// VOLUME
// ══════════════════════════════════════════════════════════════════════════════

/** 1 m³ = 35.3147 ft³ */
export const M3_TO_FT3 = 35.3147;

export const m3ToFt3 = (m3: number | string): number => num(m3) * M3_TO_FT3;
export const ft3ToM3 = (ft3: number | string): number => num(ft3) / M3_TO_FT3;

// ══════════════════════════════════════════════════════════════════════════════
// TEMPERATURE
// ══════════════════════════════════════════════════════════════════════════════

/**
 * cToF(celsius)
 * Uses numOrNull guard — returns null for invalid input.
 * Callers should check for null before using result in calculations.
 */
export const cToF = (celsius: number | string | null | undefined): number | null => {
  const c = numOrNull(celsius);
  return c === null ? null : c * 9 / 5 + 32;
};

/**
 * fToC(fahrenheit)
 * Uses numOrNull guard — returns null for invalid input.
 */
export const fToC = (fahrenheit: number | string | null | undefined): number | null => {
  const f = numOrNull(fahrenheit);
  return f === null ? null : (f - 32) * 5 / 9;
};

// ══════════════════════════════════════════════════════════════════════════════
// ENERGY / POWER
// ══════════════════════════════════════════════════════════════════════════════

/** 1 W = 3.41214 BTU/hr (NIST exact) */
export const W_TO_BTU_HR  = 3.41214;
export const BTU_HR_TO_W  = 1 / W_TO_BTU_HR;

export const wToBtuHr  = (w: number | string): number      => num(w)      * W_TO_BTU_HR;
export const btuHrToW  = (btuHr: number | string): number  => num(btuHr)  * BTU_HR_TO_W;

/** W/ft² → BTU/hr·ft² (same scalar as W → BTU/hr, area cancels) */
export const wPerFt2ToBtuHrFt2 = (wPerFt2: number | string): number => num(wPerFt2) * W_TO_BTU_HR;

/** 1 kW = 3412.14 BTU/hr */
export const KW_TO_BTU_HR = 3412.14;
export const BTU_HR_TO_KW = 1 / KW_TO_BTU_HR;

export const kwToBtuHr = (kw: number | string): number     => num(kw)     * KW_TO_BTU_HR;
export const btuHrToKw = (btuHr: number | string): number  => num(btuHr)  * BTU_HR_TO_KW;

/** 1 ton of refrigeration = 12,000 BTU/hr */
export const BTU_PER_TON  = 12000;
export const BTU_HR_TO_TR = 1 / BTU_PER_TON;
export const TR_TO_BTU_HR = BTU_PER_TON;

export const btuHrToTr = (btuHr: number | string): number => num(btuHr) * BTU_HR_TO_TR;
export const trToBtuHr = (tr: number | string): number    => num(tr)    * TR_TO_BTU_HR;

export const kwToTr = (kw: number | string): number => btuHrToTr(kwToBtuHr(kw));
export const trToKw = (tr: number | string): number => btuHrToKw(trToBtuHr(tr));

/** 1 MBH = 1000 BTU/hr */
export const BTU_HR_TO_MBH = 1 / 1000;

export const btuHrToMbh = (btuHr: number | string): number => num(btuHr) / 1000;

// ══════════════════════════════════════════════════════════════════════════════
// HUMIDITY
// ══════════════════════════════════════════════════════════════════════════════

/** 7000 grains = 1 lb water vapour */
export const GR_PER_LB = 7000;

/** Converts humidity ratio from gr/lb to lb/lb. */
export const grToLb = (grains: number | string): number => num(grains) / GR_PER_LB;

/** Converts humidity ratio from lb/lb to gr/lb. */
export const lbToGr = (lb: number | string): number => num(lb) * GR_PER_LB;

// ── g/kg ↔ gr/lb ─────────────────────────────────────────────────────────────
// ASHRAE Fundamentals tables (Ch.1, 2, 6) express humidity ratio in g/kg.
// The app uses gr/lb internally. Conversion is exactly ×7 (7000/1000).

export const gPerKgToGrPerLb = (gPerKg: number | string): number => num(gPerKg) * 7;
export const grPerLbToGPerKg = (grPerLb: number | string): number => num(grPerLb) / 7;

// ── kg/kg (= lb/lb) ↔ gr/lb ──────────────────────────────────────────────────
// kg/kg and lb/lb humidity ratios are numerically identical.

export const kgPerKgToGrPerLb = (kgKg: number | string): number => num(kgKg) * GR_PER_LB;
export const grPerLbToKgPerKg = (grPerLb: number | string): number => num(grPerLb) / GR_PER_LB;

// ══════════════════════════════════════════════════════════════════════════════
// AIRFLOW
// ══════════════════════════════════════════════════════════════════════════════

/** 1 m³/s = 2118.88 CFM */
export const M3S_TO_CFM = 2118.88;

export const m3sToCfm = (m3s: number | string): number => num(m3s) * M3S_TO_CFM;
export const cfmToM3s = (cfm: number | string): number => num(cfm) / M3S_TO_CFM;

/** 1 L/s = 2.11888 CFM */
export const LS_TO_CFM = 2.11888;

export const lsToCfm = (ls: number | string): number  => num(ls)  * LS_TO_CFM;
export const cfmToLs = (cfm: number | string): number => num(cfm) / LS_TO_CFM;

// ══════════════════════════════════════════════════════════════════════════════
// PRESSURE
// ══════════════════════════════════════════════════════════════════════════════

/** 1 inHg = 3386.39 Pa */
export const IN_HG_TO_PA = 3386.39;

export const inHgToPa = (inHg: number | string): number => num(inHg) * IN_HG_TO_PA;
export const paToInHg = (pa: number | string): number   => num(pa)   / IN_HG_TO_PA;

/** 1 inWG (in. water gauge) = 249.089 Pa */
export const IN_WG_TO_PA = 249.089;

export const inWgToPa = (inWg: number | string): number => num(inWg) * IN_WG_TO_PA;
export const paToInWg = (pa: number | string): number   => num(pa)   / IN_WG_TO_PA;

// ── Pa ↔ hPa ──────────────────────────────────────────────────────────────────
// Bridges psychro.js internals (hPa) with SI output layer (Pa).

/** 1 hPa = 100 Pa */
export const HPA_TO_PA = 100;
export const PA_TO_HPA = 1 / HPA_TO_PA;

export const hpaToPa = (hpa: number | string): number => num(hpa) * HPA_TO_PA;
export const paToHpa = (pa: number | string): number  => num(pa)  * PA_TO_HPA;

// ── hPa ↔ inHg ────────────────────────────────────────────────────────────────
// Bridges sitePressure() output (hPa) with ASHRAE climate data tables (inHg).

/** 1 hPa = 0.029530 inHg */
export const HPA_TO_IN_HG = 0.029530;

export const hpaToInHg = (hpa: number | string): number  => num(hpa)  * HPA_TO_IN_HG;
export const inHgToHpa = (inHg: number | string): number => num(inHg) / HPA_TO_IN_HG;

// ══════════════════════════════════════════════════════════════════════════════
// MASS FLOW / PIPE SIZING
// ══════════════════════════════════════════════════════════════════════════════

/** 1 GPM = 3.78541 L/min */
export const GPM_TO_LPM = 3.78541;

export const gpmToLpm = (gpm: number | string): number => num(gpm) * GPM_TO_LPM;
export const lpmToGpm = (lpm: number | string): number => num(lpm) / GPM_TO_LPM;

// ── Steam / humidifier mass flow ──────────────────────────────────────────────

export const lbPerHrToKgPerHr = (lbHr: number | string): number => num(lbHr) * 0.453592;
export const kgPerHrToLbPerHr = (kgHr: number | string): number => num(kgHr) / 0.453592;

// ══════════════════════════════════════════════════════════════════════════════
// LENGTH
// ══════════════════════════════════════════════════════════════════════════════

/** 1 ft = 304.8 mm */
export const FT_TO_MM = 304.8;

export const ftToMm = (ft: number | string): number     => num(ft)   * FT_TO_MM;
export const mmToFt = (mm: number | string): number     => num(mm)   / FT_TO_MM;
export const inchToMm = (inch: number | string): number => num(inch) * 25.4;
export const mmToInch = (mm: number | string): number   => num(mm)   / 25.4;

// ══════════════════════════════════════════════════════════════════════════════
// DISPLAY FORMATTERS
// ══════════════════════════════════════════════════════════════════════════════

/** Converts m² to ft² and returns a display string. */
export const formatArea = (m2: number | string, dp: number = 0): string =>
  `${m2ToFt2(m2).toLocaleString(undefined, {
    minimumFractionDigits: dp,
    maximumFractionDigits: dp,
  })} ft²`;

/** BTU/hr → TR, 2 decimal places. */
export const formatTR = (btuHr: number | string): string =>
  `${btuHrToTr(btuHr).toFixed(2)} TR`;

/** BTU/hr → kW, 2 decimal places. */
export const formatKW = (btuHr: number | string): string =>
  `${btuHrToKw(btuHr).toFixed(2)} kW`;



// ══════════════════════════════════════════════════════════════════════════════
// REGRESSION / SANITY CHECK REFERENCE VALUES
// ══════════════════════════════════════════════════════════════════════════════

/**
 * PSYCHRO_SANITY_CHECKS
 *
 * Expected output values from psychro.js v2.0 (Hyland-Wexler) at known conditions.
 * Use these in test-utils.jsx or any future unit test suite to catch regressions.
 *
 * ── Reference value verification (v2.1 corrections) ──────────────────────────
 *
 *   reference.enthalpyBtuLb: corrected 28.10 → 25.30
 *     ASHRAE Eq.30 at 70°F, W=0.007782 lb/lb:
 *       h = 0.240×70 + 0.007782×(1061 + 0.444×70) = 16.80 + 8.50 = 25.30 BTU/lb
 *     The previous value of 28.10 is correct for 75°F, 50%RH — not 70°F.
 *
 *   denver.sensibleFactor: corrected 0.899 → 0.889
 *     altCf = (1 − 6.8754×10⁻⁶ × 5280)^5.2559 = 0.8234
 *     sensibleFactor = 1.08 × 0.8234 = 0.8893
 *     Previous value of 0.899 was outside its own stated ±0.005 tolerance.
 *
 *   denver.grains: corrected 23.6 → 58.2
 *     P_site = 14.696 × 0.8234 = 12.098 psia; Pws(95°F) = 0.7978 psia
 *     Pw = 0.20 × 0.7978 = 0.15956 psia
 *     W = 0.62198 × 0.15956 / (12.098 − 0.15956) = 58.2 gr/lb
 *     Previous value of 23.6 was irreconcilable with these inputs.
 *
 * The critical dry condition row documents the Magnus vs H-W discrepancy.
 * After deploying psychro.js v2.0, recalculate all load summaries for
 * projects targeting <5%RH — the dew point correction affects humidification
 * load sizing by approximately 10–15%.
 */

interface ToleranceExpected {
  expected: number;
  tolerance: number;
  note?: string;
}

interface SanityCheck {
  db: number;
  rh: number;
  elev: number;
  grains?: ToleranceExpected;
  dewPointF?: ToleranceExpected;
  dewPointC?: ToleranceExpected;
  wetBulbF?: ToleranceExpected;
  enthalpyBtuLb?: ToleranceExpected;
  specVolFt3Lb?: ToleranceExpected;
  sensibleFactor?: ToleranceExpected;
}

export const PSYCHRO_SANITY_CHECKS: Record<string, SanityCheck> = {
  reference: {
    db: 70, rh: 50, elev: 0,
    grains:         { expected: 54.8,  tolerance: 0.5  },
    dewPointF:      { expected: 50.8,  tolerance: 0.3  },
    wetBulbF:       { expected: 58.6,  tolerance: 0.2  },
    enthalpyBtuLb:  { expected: 25.30, tolerance: 0.1  },
    specVolFt3Lb:   { expected: 13.51, tolerance: 0.05 },
  },
  criticalDry: {
    db: 70, rh: 1, elev: 0,
    grains:         { expected: 1.07,  tolerance: 0.05 },
    dewPointF:      { expected: -35.1, tolerance: 0.5, note: 'FROST point. Magnus returned −31.2°F — 3.9°F error.' },
    dewPointC:      { expected: -37.3, tolerance: 0.3, note: 'Magnus returned −35.1°C — +2.2°C error at this condition.' },
    wetBulbF:       { expected: 34.0,  tolerance: 1.0  },
    enthalpyBtuLb:  { expected: 16.81, tolerance: 0.1  },
  },
  denver: {
    db: 95, rh: 20, elev: 5280,
    grains:         { expected: 58.2,  tolerance: 0.5  },
    sensibleFactor: { expected: 0.889, tolerance: 0.005 },
  },
};